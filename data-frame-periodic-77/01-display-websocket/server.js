...
const server = http.createServer((req, res) => {
    ...
    fs.readFile(filePath, (err, data) => {
        if (err) {
            console.log("can not serve file: ", filePath);            
            res.statusCode = 404;
            res.end('Not Found');
        } else {
...
wss.on('connection', (ws) => {
    ...
    ws.on('message', (messageData) => {
        ...
        if (message === 'START-STREAMING') {
            console.log("client requested to stream.",frameIndex, performance.now());
            isStreaming = true;
            frameIndex = 0;
            const intervalId = setInterval(() => {
                if (isStreaming) {
                    if (frameIndex < frames.length) {
                        ...
                        frameIndex=0; return;//comment this if you want to limit data sending. 
                        console.log("finished sending all frames.",frameIndex, performance.now());
